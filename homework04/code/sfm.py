...
def get_init_extrinsics(image_id1: str, image_id2: str, intrinsics: np.ndarray) -> (np.ndarray, np.ndarray):
    ...
    for R, t in P2_list:
        ...
        C = -R.T @ t
        for Xi in X.T:
            if np.sign(np.linalg.det(P1[:, :3]) * P1[2, :3] @ Xi) == 1 and np.sign(
                    np.linalg.det(P2[:, :3]) * P2[2, :3] @ (Xi - C)) == 1:
                cur_val += 1
        if cur_val > max_val:
            max_val = cur_val
...
def solve_pnp(image_id: str, point2d_idxs: np.ndarray, all_points3d: np.ndarray, point3d_idxs: np.ndarray,
    ...
    for _ in range(num_ransac_iterations):
        ...
        retval, rotationvec, tvec = cv2.solvePnP(objectPoints=selected_pts3d, imagePoints=selected_pts2d, cameraMatrix=intrinsics, distCoeffs=None, flags=cv2.SOLVEPNP_ITERATIVE)
